    ...
    while (tag) {
        ...
        if (tag->error) {
            ...
            switch (tag->type) {
                ...
                case TYPE_SRATIONAL:
                    for (i = 0; i < (int) tag->count; i++) {
                        PRINTF(p, "%d/%d ", (int) tag->numData[i * 2],
                               (int) tag->numData[i * 2 + 1]);
                    }
                    break;
...
static void *parseIFD(FILE *fp,
    ...
    if (ifdType == IFD_0TH) {
        ...
        if (seekToRelativeOffset(fp,
                                 sizeof(TIFF_HEADER) + sizeof(short) +
                                 sizeof(IFD_TAG) * tagCount) != 0 ||
